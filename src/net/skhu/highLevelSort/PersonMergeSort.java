...
public class PersonMergeSort {
	...
	static PersonNameComparator pmc = new PersonNameComparator();
    ...
    static void merge(Person[] a, int start, int middle, int end) {
        ...
        Person[] temp = new Person[length];   // 병합 결과를 저정할 임시 배열을 생성한다.
        ...
        while (index1 <= middle && index2 <= end) { // 병합할 값이 앞부분과 뒷부분에 둘 다 남아 있다면
            if (pmc.compare(a[index1], a[index2])<0)// 앞부분과 뒷부분의 선두의 값을 비교하여
