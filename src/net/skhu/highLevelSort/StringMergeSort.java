...
public class StringMergeSort {
    ...
    static void merge(String[] a, int start, int middle, int end) {
        ...
        String[] temp = new String[length];   // 병합 결과를 저정할 임시 배열을 생성한다.
        ...
        while (index1 <= middle && index2 <= end) { // 병합할 값이 앞부분과 뒷부분에 둘 다 남아 있다면
            if (a[index1].compareTo(a[index2])<0)              // 앞부분과 뒷부분의 선두의 값을 비교하여
    ...
    public static void main(String[] args) {
        String[] a = {"kim","lee","park","choi","jung","yoon","yoo","kang","kwak"};
